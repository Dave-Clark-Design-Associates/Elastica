...
class MatchTest extends BaseTest
{
    ...
    public function testMatchZeroTerm()
    {
        ...
        $query->setFieldZeroTermsQuery('name', ElasticMatch::ZERO_TERM_ALL);
    ...
    {
        $match = new ElasticMatch(null, 'values');
        $this->assertEquals(['match' => []], $match->toArray());

        $match = new ElasticMatch('field', null);
        $this->assertEquals(['match' => []], $match->toArray());

        $match1 = new ElasticMatch('field', 'values');
        $match2 = new ElasticMatch();
